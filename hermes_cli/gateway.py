...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}{cli_name()} gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}{cli_name()} gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print(f"  This is usually a manual foreground/tmux/nohup run, so `{cli_name()} gateway`")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info(f"    {cli_name()} gateway uninstall")
    print_info(f"    sudo {cli_name()} gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                ...
                print_info(f"  After setup, run: sudo {cli_name()} gateway install --system --run-as-user <your-user>")
            print_info(f"  Then start it with: sudo {cli_name()} gateway start --system")
            return scope, False
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info(f"         sudo {cli_name()} gateway uninstall --system")
    print_info(f"         {cli_name()} gateway install")
    print_info(f"         {cli_name()} gateway start")
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}{cli_name()} gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}{cli_name()} gateway status{scope_flag}             # Check status")
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            f"check `{cli_name()} gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                f"check `{cli_name()} gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}{cli_name()} gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}{cli_name()} gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}{cli_name()} gateway start{scope_flag}")
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    f"Use DM pairing (unknown users request access, you approve with '{cli_name()} pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(f"  Approve with: {cli_name()} pairing approve <platform> <code>")
                else:
                    print_info(f"  Skipped — configure later with '{cli_name()} gateway setup'")
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info(f"  Install them, then rerun `{cli_name()} gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(f"  Unknown DM users can request access and you approve them with `{cli_name()} pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(f"  Unknown users can request access; approve with `{cli_name()} pairing approve`.")
    elif access_idx == 1:
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info(f"Start manually: {cli_name()} gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info(f"  You can try manually: {cli_name()} gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info(f"  You can install later: {cli_name()} gateway install")
                    if supports_systemd_services():
                        print_info(f"  Or as a boot-time service: sudo {cli_name()} gateway install --system")
                    print_info(f"  Or run in foreground:  {cli_name()} gateway run")
            elif is_wsl():
        ...
        print()
        print_info(f"No platforms configured. Run '{cli_name()} gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print(f"Run manually: {cli_name()} gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
            ...
            print()
            print(f"To run the gateway: {cli_name()} gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print(f"Run manually: {cli_name()} gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print(f"Stop manual runs with: {cli_name()} gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print(f"Or run the gateway directly: {cli_name()} gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print(f"    {cli_name()} gateway restart")
                    return
            ...
            if service_configured:
                ...
                print(f"  Fix the service, then retry: {cli_name()} gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print(f"  {cli_name()} gateway install")
                else:
                    print("To install as a service:")
                    print(f"  {cli_name()} gateway install")
                    print(f"  sudo {cli_name()} gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(f"  {cli_name()} gateway install  # Install as user service")
                    print(f"  sudo {cli_name()} gateway install --system  # Install as boot-time system service")
