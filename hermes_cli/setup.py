...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, f"run '{cli_name()} setup tts'"))
    else:
    ...
    print()
    print(f"   {color(f'{cli_name()}', Colors.GREEN)}              Start chatting")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                f"list? (recommended after `{cli_name()} update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        print_info(
            f"   Re-run `{cli_name()} slack manifest --write` anytime to refresh after "
        ...
        print_info(
            ...
            f"{cli_name()} slack manifest --write"
        )
    ...
    if any_messaging:
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info(f"   {cli_name()} gateway              # Run in foreground")
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            f"Skipping migration. You can run it later with: {cli_name()} claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            f"Migration cancelled. You can run it later with: {cli_name()} claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use {cli_name()} claw migrate --overwrite to force).")
    if skipped:
